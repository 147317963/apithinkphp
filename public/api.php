...
$module_dir = opendir(APP_PATH);//打开目录
while(($module_name = readdir($module_dir)) !== false) {
	$module_path = APP_PATH . DIRECTORY_SEPARATOR . $module_name;    //构建子目录路径
	if(is_dir($module_path)) { //路径是否存在
		$module = strtolower($module_name);//转换成小写
		$module_child_dir = opendir($module_path);//打开目录
		while(($module_child_name = readdir($module_child_dir)) !== false) {//目录是否存在
...
$swagger = array(
	'swagger'	=> '2.0',
	'info'		=> array(
		'description'	=> 'Creator by ZhuxinQin',
		'version'		=> '1.0.0',
		'title'			=> '金都API接口管理',
		'termsOfService'=> '',
//		'contact'		=> array(
