...
class Bjsc extends Controller
{
    public function bjsc_js(){



    }
    //获得开奖结果1
    public function auto_bjsc_1(){
        ...
        $url="http://api.1680210.com/pks/getPksHistoryList.do?lotCode=10001";
        $curl=curl_init();
    ...
    public function auto_bjsc_2(){
        ...
        $url="http://z.apiplus.net/newly.do?token=67f176514dfd19dc&code=bjpk10&rows=1&format=json";
        $curl=curl_init();
        curl_setopt($curl, CURLOPT_URL, $url);//登录提交的地址
        curl_setopt($curl, CURLOPT_HEADER, 0);//是否显示头信息
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);//是否自动显示返回的信息
