...
class Member extends Base
{
    ...
    public function group_status()
    {
        ...
        $status = $membergroup->where(['id'=>$id])->value('status');//判断当前状态情况
        if($status==1)
        {
            $flag = $membergroup->where(['id'=>$id])->setField(['status'=>2]);
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => '已禁止']);
        }
        else
        {
            $flag = $membergroup->where(['id'=>$id])->setField(['status'=>1]);
    ...
    public function index(){
        ...
        $key = input('key');
        $map[] = ['status','in','1,2'];//0未删除，1已删除
        if($key&&$key!=="")
        {
            $map[] = ['username|name|qq','like',"%" . $key . "%"];
        }
    ...
    public function member_status()
    {
        $member = new MemberModel();
        $id = input('param.id');
        $status = $member->where('id',$id)->value('status');//判断当前状态情况
        $username=$member->where(['id'=>$id])->value('username');
        if($status==1)
        {
            $flag = $member->where('id',$id)->setField(['status'=>2]);
            writelog(session('id'),session('username'),'禁止【'.$username.'】登陆成功',1);
            return json(['code' => 1, 'data' => $flag['data'], 'msg' => '已禁止']);
        }
        else
        {
            $flag = $member->where('id',$id)->setField(['status'=>1]);
            writelog(session('id'),session('username'),'开启【'.$username.'】登陆成功',1);
            return json(['code' => 0, 'data' => $flag['data'], 'msg' => '已开启']);
        }
