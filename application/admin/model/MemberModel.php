...
class MemberModel extends Model
{
    ...
    {
        return $this
            ->field('think_member.*,address,number')
    ...
    public function insertMember($param)
    {
        try{
            ...
            if(false === $result){            
                ...
                $username=$this->field('username')->where(['username'=>$param['username']])->value('username');
                writelog(session('id'),session('username'),'添加【'.$username.'】账户成功',1);
    ...
    public function editMember($param)
    {
        try{
            ...
            if(false === $result){            
                ...
                $username=$this->field('username')->where(['id'=>$param['id']])->value('username');
                writelog(session('id'),session('username'),'编辑【'.$username.'】信息成功',1);
    ...
    public function delMember($id)
    {
        try{
            $map['status']=3;
            $this->save($map, ['id' => $id]);
            $username=$this->field('username')->where(['id'=>$id])->value('username');
            writelog(session('id'),session('username'),'删除【'.$username.'】账户成功',1);
            return ['code' => 1, 'data' => '', 'msg' => '删除成功'];
        }catch( PDOException $e){
            return ['code' => 0, 'data' => '', 'msg' => $e->getMessage()];
        }
